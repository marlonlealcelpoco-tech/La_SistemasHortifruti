  ...
  async confirm(purchaseId: number, salePriceUpdates: Map<number, boolean>): Promise<{ kind: "not_found" | "already_confirmed" } | { kind: "success"; purchase: PurchaseRecord }> {
    ...
    try {
      ...
      if (!existingPayable.rows[0]) {
        const payable = await client.query<{ id: number }>(`INSERT INTO financial_entries (type, description, amount, due_date, supplier_id, source, document_number, purchase_id) VALUES ('PAYABLE', $1, $2, CURRENT_DATE, $3, 'PURCHASE', $4, $5) RETURNING id`, [`Compra ${purchaseId} - Conta a Pagar`, Number(record.total), record.supplier_id, `COMPRA-${purchaseId}`, purchaseId]);
