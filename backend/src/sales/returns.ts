...
export type ReturnInput = {
  ...
  productId: number;
  quantity: number;
  reason: "CUSTOMER_REGRET" | "WRONG_PRODUCT" | "DEFECT" | "OTHER";
  notes?: string | null;
};
...
export class SalesReturnRepository {
  ...
  async create(input: ReturnInput): Promise<{ returnId: number; creditAmount: number }> {
    ...
    try {
      ...
      const sold = await client.query<{ quantity: string; unit_price: string }>(
        `SELECT quantity, unit_price FROM sale_items WHERE sale_id = $1 AND product_id = $2 FOR UPDATE`, [input.saleId, input.productId]
      );
      ...
      const ret = await client.query<{ id: number }>(
        `INSERT INTO sales_returns (sale_id, customer_id, product_id, quantity, amount, reason, notes)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`, [input.saleId, input.customerId, input.productId, input.quantity, creditAmount, input.reason, input.notes ?? null]
      );
      await client.query("UPDATE stock SET quantity = quantity + $2 WHERE product_id = $1", [input.productId, input.quantity]);
      await client.query(
        `INSERT INTO stock_movements (product_id, type, quantity, reference, notes, performed_by, previous_quantity, resulting_quantity)
         SELECT $1, 'RETURN', $2, $3, $4, NULL, quantity - $2, quantity
           FROM stock WHERE product_id = $1`, [input.productId, input.quantity, `DEVOLUCAO-${ret.rows[0].id}`, "Devolução vinculada à venda original"]
      );
