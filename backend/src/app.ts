...
import { InventoryRepository } from "./estoque/repository.js";
import { registerInventoryRoutes } from "./estoque/routes.js";
...
export function buildApp(environment: Environment, pool: Pool) {
  ...
  registerSupplierRoutes(app, users, suppliers);
  registerProductCadastroRoutes(app, users, productService);
  ...
  app.setErrorHandler((error, _request, reply) => {
    ...
    if (environment.NODE_ENV === "test") {
      ...
      return reply.code(500).send({ message: "Erro interno do servidor.", details: message });
    }
