...
/** Pure alert builder. Persistence and notification delivery remain application concerns. */
export function buildManagementAlerts(
  ...
  if (metrics.overdueReceivable > receivableLimit) {
    alerts.push({
      ...
      description: `Existem R$ ${metrics.overdueReceivable.toFixed(2)} em recebíveis vencidos.`,
      source: 'financial_receivables'
    });
  }
  ...
  if (metrics.overduePayable > payableLimit) {
    alerts.push({
      ...
      description: `Existem R$ ${metrics.overduePayable.toFixed(2)} em obrigações vencidas.`,
      source: 'financial_payables'
    });
  }
  ...
  if (metrics.lowStockItems > lowStockLimit) {
    alerts.push({
      ...
      description: `${metrics.lowStockItems} produto(s) estão abaixo do estoque mínimo.`,
      source: 'inventory'
    });
  }
