...
export const ADJUSTMENT_REASONS = ["LOSS", "EXPIRED", "DAMAGED", "THEFT", "BREAKAGE", "INTERNAL_USE", "MISSING", "INVENTORY_DIFFERENCE", "ENTRY_CORRECTION", "FOUND_SURPLUS", "OTHER"] as const;
...
export class InventoryAdjustmentRepository {
  ...
  async createCountAdjustment(input: InventoryCount, userId: number): Promise<{ movementId: number; previousQuantity: number; newQuantity: number }> {
    ...
    try {
      ...
      const updated = await client.query<{ quantity: string }>("UPDATE stock SET quantity = $2 WHERE product_id = $1 RETURNING quantity", [input.productId, input.countedQuantity]);
      const movement = await client.query<{ id: number }>(`INSERT INTO stock_movements (product_id, type, quantity, reference, notes, adjustment_reason, performed_by, previous_quantity, resulting_quantity) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`, [input.productId, difference > 0 ? "ENTRY" : "EXIT", Math.abs(difference), `INVENTORY-${Date.now()}`, input.notes ?? "Ajuste originado por conferência de inventário", input.reason, userId, previousQuantity, Number(updated.rows[0].quantity)]);
