...
const movementSchema = z.object({
  ...
  if (value.type !== "ADJUSTMENT" && value.quantity <= 0) context.addIssue({ code: z.ZodIssueCode.custom, message: "Entradas e saídas devem ter quantidade positiva.", path: ["quantity"] });
});
