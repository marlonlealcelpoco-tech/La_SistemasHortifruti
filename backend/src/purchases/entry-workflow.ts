...
export type PurchaseEntryPayment = {
...
/** A product reaches stock through a purchase entry/document, manual or XML. */
export function validatePurchaseEntryBeforeConfirmation(entry: PurchaseEntryDraft): void {
  ...
  for (const item of entry.items) {
    if (!item.code.trim() || !item.description.trim()) throw new Error("Código e descrição são obrigatórios para todos os itens.");
    ...
    if (item.productId === null) throw new Error(`Cadastre/vincule o produto ${item.description} antes de confirmar a entrada.`);
  }
  for (const payment of entry.payments) {
    if (!Number.isInteger(payment.installmentNumber) || payment.installmentNumber < 1) throw new Error("Número de parcela inválido.");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(payment.dueDate) || payment.amount <= 0) throw new Error("Parcela financeira inválida.");
  }
}
